...
fn filter_response(
    ...
    let length_match = filters
        .filter_reponse_len
        .as_ref()
        .is_some_and(|leng| leng.contains(&res_len));

    // Filter by response body containing specific text
    let content_match = filters
        .contain
        .as_ref()
        .is_some_and(|match_cont| match_cont.contains(res_body));
...
async fn craft_request(args: cli::Args, client: Arc<Client>, word: String) {
    ...
    let res = match args_clone.method {
        Some(HTTPMethods::GET) | None => match client.get(url).headers(headers_hash).send().await {
            Ok(body) => body,
            Err(_) => return,
        },
